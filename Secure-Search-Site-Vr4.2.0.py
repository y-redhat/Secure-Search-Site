...
# -*- coding: utf-8 -*-
# ===================================================================
# SSS (Secure-Search-Site) WebSocket Stealth Proxy Vr4.2.0 (Colab Stable)
# Fixes: JS Stack Overflow, Tor Timeout Freeze, UI Feedback
# ===================================================================
import os
import sys
import time
...
app = Flask(__name__)
sock = Sock(app)
limiter = Limiter(key_func=get_remote_address, app=app, default_limits=["200 per day", "50 per hour"], storage_uri="memory://")
...
if not _PASSWORD_HASH_ENV:
    if _IS_COLAB:
        ...
        print(f"[!] SSS_PASSWORD_HASH not set. Using default: {default_pw}")
    else:
...
# 【重要】Colab環境ではTorを無効化し、タイムアウトを厳格化してフリーズを防ぐ
if _IS_COLAB:
    _PROXIES = None
    _REQ_TIMEOUT = (5, 15)  # (接続タイムアウト, 読み取りタイムアウト)
else:
...
_BLOCKED_HOSTS = {"localhost", "127.0.0.1", "0.0.0.0", "::1", "[::1]", "169.254.169.254", "metadata.google.internal"}
...
def _safe_fetch(url: str):
    ...
    for _ in range(_MAX_REDIRECTS):
        ...
        try:
            resp = requests.get(
                current_url, proxies=_PROXIES, timeout=_REQ_TIMEOUT,
            ...
            # Tor失敗時は直線接続にフォールバック
            try:
                resp = requests.get(current_url, timeout=_REQ_TIMEOUT, allow_redirects=False, stream=True, headers={"User-Agent": "Mozilla/5.0"})
...
function _uint8ToBase64(u8a) {
    ...
    for (let i = 0; i < u8a.length; i += chunkSize) {
        const chunk = u8a.subarray(i, i + chunkSize);
...
async function _enc(p) {
    try {
        ...
        alert("Encryption failed (Check HTTPS): " + err.message);
        return null;
    }
}
...
async function _load() {
    if (!_ws || _ws.readyState !== WebSocket.OPEN) {
        alert("WebSocket is not connected. Please reload.");
        return;
    }
